...
def rabin_encrypt(n, message):
    ...
    message_length = len(message_bytes)  # Get the length of the message

    # Prepend the length of the message before the actual content (use 4 bytes to store the length)
    padded_message = message_length.to_bytes(4, 'big') + message_bytes

    # Convert the padded message to an integer for encryption
...
def rabin_decrypt(p, q, n, ciphertext):
    ...
    for root in possible_roots:
        try:
            ...
            decrypted_bytes = root.to_bytes((root.bit_length() + 7) // 8, 'big')

            # Extract the first 4 bytes to determine the message length
            message_length = int.from_bytes(decrypted_bytes[:4], 'big')

            # Extract the message content using the length
            decrypted_message = decrypted_bytes[4:4 + message_length].decode()
